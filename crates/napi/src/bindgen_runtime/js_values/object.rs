use crate::{bindgen_prelude::*, check_status, sys, type_of_, JsObject, ValueType};
...
impl Object {
  ...
  pub fn get<K: AsRef<str>, V: FromNapiValue>(&self, field: K) -> Option<V> {
    let c_field = CString::new(field.as_ref()).unwrap();
    ...
    unsafe {
      ...
      let ty = type_of_!(self.0.env, ret);
  ...
  pub fn set<K: AsRef<str>, V: ToNapiValue>(&mut self, field: K, val: V) {
    let c_field = CString::new(field.as_ref()).unwrap();

    unsafe {
      if let Ok(napi_val) = V::to_napi_value(self.0.env, val) {
        check_status_or_throw!(
          self.0.env,
        ...
        JsError::from(Error::new(
          ...
          format!("Failed to set [{}] on obj", field.as_ref()),
        ))
        .throw_into(self.0.env);
      }
    }
  }
